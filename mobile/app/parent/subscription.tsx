...
import { notifySubscriptionActivated } from "../services/notifications";
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        await notifySubscriptionActivated(planName);
        Alert.alert("🎊 Success!", `${planName} plan is now active. Enjoy all premium features!`);
      } else {
        Alert.alert("Error", res.message || "Failed to update subscription.");
      }
    } catch (e) {
      Alert.alert("Error", "Subscription failed. Please check backend connection.");
    } finally {
      ...
      {loading ? (
        ...
        >
          ...
          <View style={styles.plansContainer}>
            {/* Basic Plan */}
            <View style={[styles.glassCard, subscription.plan === "free" && styles.activePlanBorder]}>
              ...
              <TouchableOpacity 
                style={[styles.basicBtn, subscription.plan === "free" && styles.activePlanBtn]} 
                disabled={subscription.plan === "free" || subscribing}
                onPress={() => handleSubscribe("free")}
              >
                <Text style={[styles.basicBtnText, subscription.plan === "free" && styles.activePlanBtnText]}>
                  {subscription.plan === "free" ? "Current Plan" : "Downgrade to Basic"}
                </Text>
              </TouchableOpacity>
            </View>

            {/* Pro Plan (Recommended) */}
            <View style={[styles.glassCard, styles.proCard, subscription.plan === "pro" && styles.activePlanBorderPro]}>
              ...
              <TouchableOpacity 
                style={[styles.proBtn, subscription.plan === "pro" && styles.activeProPlanBtn]} 
                activeOpacity={0.8}
                disabled={subscription.plan === "pro" || subscribing}
